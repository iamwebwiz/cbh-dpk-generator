const { deterministicPartitionKey } = require('./dpk-generator');
...
describe('deterministricPartitionKey', () => {
    ...
    it('should return the partition key specified on the event if the key exists', () => {
        expect(deterministicPartitionKey({ partitionKey: 'testPartitionKey' })).toBe('testPartitionKey');
    });
    ...
    it('should return the sha3-512 hash of the partition key if it is longer than the maximum partition key length - 256', () => {
        ...
        expect(deterministicPartitionKey({ partitionKey: longPartitionKey })).toBe(hash);
    });
});